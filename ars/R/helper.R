...
#' \code{w} numeric vector of area weights.
...
beta_u_x <- function (m, b, bounds) {
  ...
  non_zero_boolean_summation <-
    sum((b_selection_finite[non_zero_boolean]
         / m[non_zero_boolean]) *
  ...
  ## Normalize beta
  beta <-
    b_selection_finite / (non_zero_boolean_summation + summation_z)
  ...
  weight <-
    ifelse(non_zero_boolean, (beta / m) * (exp(m * bounds[2:length_bounds]) -
           ...
           beta * (bounds[2:length_bounds] - bounds[1:length_bounds - 1]))
  return (list(beta = beta, w = ifelse((weight > 0) &
                                         is.finite(weight), weight, 0.0
  )))
}
...
#' @return Throws an error if the algorithm has reached the maximum number of
#' iterations.
...
#' @return \code{TRUE} if \code{x} is within given bounds, \code{FALSE} otherwise.
