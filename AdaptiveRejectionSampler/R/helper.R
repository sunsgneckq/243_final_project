...
slope_intercept_l_j <- function(x, h) {
  ...
  return (list(
    m = (h[2:length_h] - h[1:length_h - 1]) /
      (x[2:length_h] - x[1:length_h - 1]),
    b = (x[2:length_h] * h[1:length_h - 1] - x[1:length_h -
                                                 1] *
           h[2:length_h]) / (x[2:length_h] - x[1:length_h -
                                                 1])
  ))
}
...
beta_u_x <- function(m, b, bounds) {
  ...
  # treat m = 0 case
  sum_none_zero <-
    sum((exp_b_selection[none_zero] / m[none_zero]) *
          (exp(m[none_zero] * bounds[2:length_bounds][none_zero]) -
             exp(m[none_zero] * bounds[1:length_bounds -
                                         1][none_zero])))
