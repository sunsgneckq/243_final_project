...
intercept_z_j <- function(x, h, dh, bounds) {
  ...
  return (append(bounds[1],
                 append((h[2:length_h] - h[1:length_h-1] -
                           ...
                           x[1:length_h-1] * dh[1:length_h-1]) / 
                          (dh[1:length_h-1] - dh[2:length_h]), bounds[2])))
}
...
sampling_x <- function(N, beta, m, weights, bounds) {
  ...
  x_CDF <- ifelse(m[sampling] != 0, 
                  (1/m[sampling])*log((m[sampling]*weights[sampling]/beta[sampling])*random_N + 
                                        exp(m[sampling]*bounds[sampling])), 
